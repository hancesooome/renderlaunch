...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  hydrate: async () => {
    try {
      ...
      set({project: restored ?? get().project, hydrated: true, saveStatus: 'saved'});
    } catch { set({hydrated: true, saveStatus: 'error'}); }
  },
  updateProject: recipe => set(state => ({
    past: [...state.past.slice(-49), state.project], future: [], saveStatus: 'unsaved',
    project: produce(state.project, draft => { recipe(draft); draft.updatedAt = new Date().toISOString(); }),
  })),
  setFrame: frame => set(state => ({currentFrame: Math.round(clamp(frame, 0, state.project.canvas.durationInFrames - 1))})),
  ...
  redo: () => set(state => {
    ...
    return {project: next, past: [...state.past, state.project], future: state.future.slice(1), saveStatus: 'unsaved'};
  }),
  persist: async () => {
    set({saveStatus: 'saving'});
    try { await saveProject(get().project); set({saveStatus: 'saved'}); }
